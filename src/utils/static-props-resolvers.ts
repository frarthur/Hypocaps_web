...
export async function resolveStaticProps(urlPath: string, data: { pages: ContentObject[]; objects: ContentObject[]; props: Record<string, unknown> }): Promise<Record<string, unknown>> {
    ...
    return mapDeepAsync(
        ...
        async (value: unknown, _keyPath: (string | number)[], _stack: unknown[]) => {
            ...
            const objectType = obj?.__metadata ? (obj.__metadata as Record<string, unknown>).modelName as string : undefined;
