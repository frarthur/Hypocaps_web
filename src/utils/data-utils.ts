...
type MapDeepIterator = (value: unknown, keyPath: (string | number)[], stack: unknown[]) => Promise<unknown>;
...
export function getAllCategoryPostsSorted(objects: ContentObject[], categoryId: string): ContentObject[] {
    const allPosts = getAllPosts(objects);
...
export function sortPosts(posts: ContentObject[]): ContentObject[] {
    return posts.sort((postA, postB) => new Date(postB.date as string).getTime() - new Date(postA.date as string).getTime());
}
...
export function resolveReferences(
    ...
    const _resolveDeep = (value: unknown, fieldNames: string[], debugContext: DebugContext): unknown => {
...
export function findObjectById(
    ...
    if (!object && debugContext) {
        ...
        const objectIndex = reverseStack.findIndex((obj) => !!(obj as ContentObject).__metadata?.relProjectPath);
        if (objectIndex >= 0) {
